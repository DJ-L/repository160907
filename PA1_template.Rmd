...
```
The mean number of steps by day is `r round(mean1,2)` and median is `r median1`
...
```
The 5-minute interval that, on average, contains the maximum number of steps is interval number `r D4[1]` with `r round(D4[2],2)` (rounded to two decimals)steps. The average daily activity pattern is dominated by the pronounced peak at this interval.
...
```
The total number of missing values is `r totMissings`. 
The mean and median number of steps by day computed on the imputed dataset is meand=`r round(meanImputed,2)` and median=`r medianImputed`. This differ slightly form the non-imputed mean and median, but not to an unexpected degree.
